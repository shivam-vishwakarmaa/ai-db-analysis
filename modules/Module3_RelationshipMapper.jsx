...
export default function RelationshipMapper({ schemaJSON, onAIReady }) {
  ...
  const roleColors = {
    fact:      { bg: '#1e3a5f', border: '#3b82f6', header: '#2563eb', text: '#93c5fd' },
    dimension: { bg: '#1a3a2a', border: '#10b981', header: '#059669', text: '#6ee7b7' },
    junction:  { bg: '#3b2a1a', border: '#f59e0b', header: '#d97706', text: '#fcd34d' },
    isolated:  { bg: '#2a2a2a', border: '#6b7280', header: '#4b5563', text: '#9ca3af' },
    unknown:   { bg: '#2a2a2a', border: '#6b7280', header: '#4b5563', text: '#9ca3af' },
  };
  ...
  const layout = useMemo(() => {
    ...
    if (total <= 3) {
      ...
      classified.forEach((t, i) => {
        positions[t.name] = { x: 100 + i * (NODE_W + NODE_PAD), y: cy - nodeHeight(t) / 2, w: NODE_W, h: nodeHeight(t), role: t.role };
      });
    } else {
      ...
      facts.forEach((t, i) => {
        ...
        const x = cx + Math.cos(angle) * (facts.length === 1 ? 0 : innerRadius) - NODE_W / 2;
        const y = cy + Math.sin(angle) * (facts.length === 1 ? 0 : innerRadius) - nodeHeight(t) / 2;
      ...
      juncs.forEach((t, i) => {
        ...
        if (fks.length >= 2 && positions[fks[0].references_table] && positions[fks[1].references_table]) {
          const p1 = positions[fks[0].references_table];
          const p2 = positions[fks[1].references_table];
          positions[t.name] = {
            x: (p1.x + p2.x) / 2,
            y: (p1.y + p2.y) / 2 + 80,
            w: NODE_W, h: nodeHeight(t), role: t.role,
          };
        } else {
  ...
  function getEdgePath(fromPos, toPos) {
    ...
    const cx1 = fromPt.x + (Math.abs(dy) > Math.abs(dx) ? offset * Math.sign(dx || 1) : 0);
    const cy1 = fromPt.y + (Math.abs(dx) >= Math.abs(dy) ? offset * Math.sign(dy || 1) : 0);
  ...
  const handleMouseDown = useCallback((e) => {
    if (e.target === svgRef.current || e.target.tagName === 'svg' || e.target.closest('.pan-area')) {
      setIsPanning(true);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white flex flex-col">
      ...
      <div className="p-4 border-b border-gray-700/50 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            ...
            {selectedTable && <span className="text-cyan-400"> · Selected: {selectedTable}</span>}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }}
            className="px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 text-xs transition-colors ml-1">Reset</button>
          ...
          <button onClick={() => setShowNarrative(!showNarrative)}
            className={`px-3 py-1 rounded text-xs transition-colors ${
              showNarrative ? 'bg-purple-600 text-white' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'
            }`}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          >
            ...
            {relationships.map((rel, i) => {
              ...
              const isHighlighted =
                selectedTable === rel.from_table || selectedTable === rel.to_table;
              ...
              return (
                ...
                >
                  ...
                  <path
                    d={path}
                    fill="none"
                    stroke={isHighlighted ? '#60a5fa' : isDimmed ? '#374151' : '#4b5563'}
                  ...
                  <g transform={`translate(${labelPos.x}, ${labelPos.y})`}>
                    ...
                    <text
                      ...
                      fill={isHighlighted ? '#93c5fd' : isDimmed ? '#4b5563' : '#9ca3af'}
                    >
                  ...
                  {/* Hover tooltip */}
                  {isHovered && (
                    <g transform={`translate(${labelPos.x}, ${labelPos.y - 28})`}>
                      <rect x={-90} y={-14} width={180} height={28} rx={6}
                        fill="#111827" stroke="#374151" strokeWidth={1} />
                      <text textAnchor="middle" dominantBaseline="central"
                        fontSize={10} fill="#d1d5db" fontFamily="sans-serif">
            ...
            {tables.map((tbl) => {
              ...
              return (
                ...
                >
                  {/* Selection glow */}
                  {isSelected && (
                    <rect x={-3} y={-3} width={pos.w + 6} height={pos.h + 6} rx={12}
                  ...
                  {/* Header */}
                  <rect width={pos.w} height={36} rx={8}
                    fill={colors.header} />
                  <rect x={0} y={28} width={pos.w} height={8}
                    fill={colors.header} />

                  {/* Table name */}
                  <text x={pos.w / 2} y={22} textAnchor="middle"
                    fontSize={12} fontWeight="bold" fill="white"
                    fontFamily="sans-serif">
                    {tbl.name.length > 22 ? tbl.name.slice(0, 20) + '…' : tbl.name}
                  </text>

                  {/* Row count badge */}
                  <text x={pos.w - 8} y={22} textAnchor="end"
                  ...
                  {keyCols.map((col, ci) => {
                    ...
                    return (
                      <g key={col.name} transform={`translate(0, ${yy})`}>
                        ...
                        {/* PK / FK badge */}
                        <rect x={pos.w - 40} y={-7} width={24} height={14} rx={3}
                          fill={isPK ? 'rgba(59,130,246,0.2)' : 'rgba(16,185,129,0.2)'}
                          stroke={isPK ? 'rgba(59,130,246,0.3)' : 'rgba(16,185,129,0.3)'}
                          strokeWidth={0.5} />
                        <text x={pos.w - 28} y={0} textAnchor="middle"
                  ...
                  {/* No keys indicator */}
                  {keyCols.length === 0 && (
                    <text x={pos.w / 2} y={52} textAnchor="middle"
        ...
        {(selectedTable || showNarrative) && (
          <aside className="w-80 min-w-[320px] border-l border-gray-700/50 bg-gray-900/60 overflow-y-auto p-4 space-y-4">
            ...
            {showNarrative && aiNarrative?.narrative && (
              <div className="rounded-xl border border-purple-500/20 bg-purple-500/5 p-4">
                <h3 className="text-sm font-semibold text-purple-400 mb-2">✦ AI Narrative</h3>
            ...
            {selectedTable && (() => {
              ...
              const colors = roleColors[role] || roleColors.unknown;
              const outgoing = relationships.filter(r => r.from_table === tbl.name);
              const incoming = relationships.filter(r => r.to_table === tbl.name);

              return (
                <>
                  {/* Table info header */}
                  <div>
                    <h3 className="text-lg font-bold text-white">{tbl.name}</h3>
                    <div className="flex gap-2 mt-2">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider border`}
                        style={{ backgroundColor: colors.bg, borderColor: colors.border, color: colors.text }}>
                        {role}
                      </span>
                      <span className="text-xs text-gray-400">{tbl.row_count?.toLocaleString()} rows</span>
                      <span className="text-xs text-gray-400">{tbl.columns?.length} cols</span>
                    </div>
                  </div>
                  ...
                  <div className="rounded-lg border border-gray-700/50 overflow-hidden">
                    <div className="px-3 py-2 bg-gray-800/60 border-b border-gray-700/50">
                      <h4 className="text-xs font-semibold text-gray-400 uppercase">All Columns</h4>
                    </div>
                    <div className="divide-y divide-gray-800/50">
                      {(tbl.columns || []).map(col => {
                        ...
                        return (
                          <div key={col.name} className="px-3 py-1.5 flex items-center gap-2 text-xs">
                            <span className="w-3">{isPK ? '🔑' : fk ? '🔗' : ''}</span>
                            <span className="text-gray-200 font-mono flex-1 truncate">{col.name}</span>
                      ...
                      <div className="p-3 space-y-1.5">
                        {outgoing.map((r, i) => (
                          <div key={i} className="text-xs text-gray-300">
                            <span className="font-mono text-blue-400">{r.from_column}</span>
                            <span className="text-gray-500"> → </span>
                            <span className="font-mono text-emerald-400">{r.to_table}.{r.to_column}</span>
                            <span className={`ml-1.5 px-1 py-0.5 rounded text-[9px] ${
                              r.inferred ? 'bg-yellow-500/15 text-yellow-400' : 'bg-emerald-500/15 text-emerald-400'
                            }`}>{r.inferred ? 'INF' : 'EXP'}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {incoming.length > 0 && (
                    <div className="rounded-lg border border-gray-700/50 overflow-hidden">
                      ...
                      <div className="p-3 space-y-1.5">
                        {incoming.map((r, i) => (
                          <div key={i} className="text-xs text-gray-300">
                            <span className="font-mono text-cyan-400">{r.from_table}.{r.from_column}</span>
                            <span className="text-gray-500"> → this.{r.to_column}</span>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      <div className="border-t border-gray-700/50 bg-gray-900/60 px-5 py-2.5">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-center gap-5 text-xs text-gray-400">
          {Object.entries(roleColors).filter(([k]) => k !== 'unknown').map(([role, c]) => (
            <span key={role} className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm" style={{ backgroundColor: c.header, border: `1px solid ${c.border}` }} />
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-6 border-t-2 border-dashed border-gray-400" /> Inferred FK
          </span>
          <span className="text-gray-700">|</span>
          <span>Click table to highlight connections · Scroll to zoom · Drag to pan</span>
        </div>
      </div>
    </div>
  );
